...
plot(zipbd, border = "white", lwd = 0.2, col = ifelse(as.character(zipbd@data$ZIPCODE) %in% zipdiff2, "red", "blue"))

#zipbd@data[which(as.character(zipbd@data$ZIPCODE) %in% zipdiff1), ]


plot(zipbd, border = "white", lwd = 0.2, col = "red")
text(coordinates(zipbd), labels = zipbd@data$ZIPCODE, cex = 0.6)
