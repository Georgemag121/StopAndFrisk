...
## finding zipcode matches for NAs
```{r}
# for stops happening on the boundary of zipcodes, probe the surrounding areas (8 points) and take the majority vote to be its zip code, the distance from the original point is close to 140 ft.
...
#abc <- cbind(sf[154,10] + 0.0001, sf[154, 9] - 0.0001)
#def <- SpatialPoints(abc, CRS("+proj=longlat +ellps=WGS84 +no_defs"))
#a <- over(def, zipbd)
```
...
Sys.time()
zipcorrect(zipmiss$lon[1:50], zipmiss$lat[1:50], zipbd)
Sys.time()
```
